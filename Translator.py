...
def translate_code(java_code):
    ...
    if first[0] not in ["int", "double", "float", "char"]:
...
try:
    ...
    reserved_words = ['for', 'while', 'int', 'double', 'float', 'char', 'boolean']
    ...
    # save python code to output.txt
    file = open('output.txt','w')
    sentence = file.write(python_code)
    file.close()